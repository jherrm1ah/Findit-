          ...
          <>
            <div className="flex items-center gap-1.5 my-3">
              {["monthly", "yearly"].map((period) => (
                ...
                >
                  {/* z-0 (not negative) — see Browse.jsx's category pill for
                      why: this card is itself wrapped in a motion.div with
                      an animated opacity, which establishes a stacking
                      context; a negative z-index here would escape behind
                      that ancestor's own white background instead of just
                      behind this label. */}
                  {billingPeriod === period && (
                    <motion.span
                      layoutId="billingPeriodPill"
            ...
            <p className="text-[18px] font-bold text-[#1E1B4B] mb-3">
              <AnimatedNumber value={price} format={(n) => naira(Math.round(n))} duration={DURATION.base} />
